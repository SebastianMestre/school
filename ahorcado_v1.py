...
def esLetra1(letra):
	...
	return (letra >= 'A' and letra <= 'Z') or (letra >= 'a' and letra <= 'z') or letra == 'Ñ' or letra == 'ñ'  
...
# jugar: String -> ResultadoPartida
# Descripción: esta es la función principal, la cual toma una palabra a adivninar, inicia y lleva adelante el juego hasta el final.
...
def jugar(palabra):
	...
	intentos = 0    # 'intentos' guarda la cantidad de intentos validos realizados por el jugador
	...
	incognita = inicializarIncognita(palabra) # Generar la secuencia de guiones que representa la palabra oculta 
													# (incognita: [Char])
	...
	while palabraIncompleta(incognita) and vidas > 0 and not fin: 	# Mientras la incognita no haya sido completada, 
		...
		if letra == bandera:		# Si la letra sugerida es el identificador de interrupción del juego, terminar el juego.
			...
			intentos += 1
			listaIndices = buscarPosiciones(letra, palabra)	# Si lo ingresado fue una letra válida; busca las posiciones en 
	...
	# Se retorna un valor ResultadoPartida en funcion del resultado de la partida y la cantidad de intentos usados		
