...
```{r fig.cap="Edad Usuarios", echo=FALSE, message=FALSE, warning=FALSE}
library(qcc)
...
Se puede observar en este histograma que la mayoría de los usuarios
estan en las franjas etarias más bajas. Más especificamente, se ve una
concentracion de usuarios en la franja de 20 a 40 años, con un pico en
el intervalo [25, 30]. Se detecta que usuarios con una edad mayor a 40
años hace un menor uso del servicio. Una aclaración importante es que el
número de personas que figuran entre 15 y 20 años es baja, ya que sólo
estamos teniendo en cuenta los mayores a 18.
...
En esta tabla podemos ver incluso mejor como llegado a los 33 años, la
frecuencia acumulada es casi del 60%.

```{r echo=FALSE, fig.cap="Edad Recorridos", message=FALSE, warning=FALSE}
...
```
<!-- TODO: agregar grafico, borre el q estaba antes pq estaba mal (ver correcciones) -->


El gráfico de ditribución de recorridos por edad nos muestra resultados
...
```{r fig.cap="Genero Usuarios", message=FALSE, echo = FALSE, warning = FALSE}
...
```{r fig.cap="Genero Recorridos", message=FALSE, echo = FALSE, warning = FALSE}
pieViajesXGenero()
```
...
Se hace primero el análisis del gráfico de la distribución de duración
con "outliers":

```{r fig.cap="Duracion Recorridos", message=FALSE, echo = FALSE, warning = FALSE}
usuarios <- read.table("usuarios8.csv", header = TRUE, sep = ",")
recorridos <- read.table("recorridos8.csv", header = TRUE, sep = ",")
usuariosrecorridos <- merge(usuarios, recorridos)
...
```{r fig.cap="Estaciones Origen", message=FALSE, echo = FALSE, warning = FALSE}
usuarios <- read.table("usuarios8.csv", header = TRUE, sep = ",")
recorridos <- read.table("recorridos8.csv", header = TRUE, sep = ",")
usuariosrecorridos <- merge(usuarios, recorridos)
...
```{r fig.cap="Estaciones Destino", message=FALSE, echo = FALSE, warning = FALSE}
...
Como se puede ver, hay unos pocos "outliers" en estos gráficos también,
...
y terminan en la misma estación. En el gráfico de abajo se ve que la
...
```{r fig.cap="Viajes por Estacion", message=FALSE, echo = FALSE, warning = FALSE}
...
```{r fig.cap="Genero por Edad", message=FALSE, echo = FALSE, warning = FALSE}
...
Al observar la gráfica se puede notar que el rango de 20 a 30 años está
...
Y con esta tabla se puden ver bien las distribuciones acumuladas.
