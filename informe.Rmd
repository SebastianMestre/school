...
```{r generousuarios1, fig.cap="Distribución de usuarios por Genero", message=FALSE, echo = FALSE, warning = FALSE, out.width="90%"}
...
```{r generousuarios2, fig.cap="Distribución de viajes por Genero", message=FALSE, echo = FALSE, warning = FALSE, out.width="90%"}
...
Vamos a analizar ahora la distribución de viajes que se realizan por día de la semana.

```{r diasdelasemana1, fig.cap="Distribución de viajes por día de la semana", message=FALSE, echo = FALSE, warning = FALSE, out.width="90%"}
...
La distribución muestra que el día en que se realizan más viajes es el lunes, mientras que los días con menor frecuencia son viernes y domingo. La dismunición de viajes en los 2 días recién mencionados puede atribuirse al hecho de que las personas pueden tender a quedarse en sus hogares a descansar durante el fin de semana, o bien realizan actividades fuera de la casa, pero se transportan de otra manera.
...
En esta tabla se observa con mayor precisión la diferencia en frecuencia entre cada día. 
...
```{r estacionesorigen1,  fig.cap="Cantidad de estaciones segun cantidad de viajes que empiezan ahí", message=FALSE, echo = FALSE, warning = FALSE, out.width="90%"}
usuarios <- read.table("usuarios8.csv", header = TRUE, sep = ",")
recorridos <- read.table("recorridos8.csv", header = TRUE, sep = ",")
usuariosrecorridos <- merge(usuarios, recorridos)
...
```{r estacionesdestino1,fig.cap="Cantidad de estaciones segun cantidad de viajes que terminan ahí", message=FALSE, echo = FALSE, warning = FALSE, out.width="90%"}
...
```{r estacionrecorridos1, fig.cap="Distribución de viajes que empiezan y terminan en el mismo lugar", message=FALSE, echo = FALSE, warning = FALSE, out.width="90%"}
...
tt <- cbind(c("F","M","Otro"),tt)

knitr::kable(tt, caption="Frecuencias Genero por Edad")
```

Y con la tabla (3) se puden ver bien las distribuciones acumuladas.

# Conclusiones
Luego de que se analizaron los distintos gráficos y tablas que se crearon para este informe se puede concluir que el sistema de EcoBicis brinda un medio de transporte muy utilizado por una gran variedad de ciudadanos de la Ciudad de Buenos Aires. La mayoría de estos se encuentran en el rango etario de entre 25 y 30 años y el género predominante entre los usarios es el femenino. El servicio se suele utilizar para viajes de entre 10 y 30 minutos, y se recorre un promedio de aproximadamente 4500 metros.
Una posible mejora al sistema es agregar más estaciones en las zonas más concurridas. Nuestro análisis denotó que, si bien la mayoría de la estaciones no parecen sufrir de sobrecarga, hay unas pocas que tienen un número particularmente alto de concurrencia, alejandose estadísticamente del resto.